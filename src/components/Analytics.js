  ...
  const computeConcurrentTasks = () => {
    ...
    return days.map((day) => {
      ...
      const count = tasks.filter((task) => {
        const createdAt = typeof task.createdAt === 'string' ? new Date(task.createdAt).getTime() : task.createdAt;
        const activeStart = task.activeStartTime || createdAt;
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50">
      <div
        ...
        onClick={onClose}
      ></div>
      <div className="relative bg-white p-4 rounded shadow-md w-[90%] max-w-4xl z-50">
        <button
          onClick={onClose}
